...
?>
	...
	<!-- Version Infos -->
        <div class="pull-right hidden-xs hidden-sm<?php if(isset($core_commit) || isset($web_commit) || isset($FTL_commit)) { ?> hidden-md<?php } ?>">
            ...
            if($core_update){ ?> <a class="alert-link animated infinite text-red lookatme" href="https://github.com/pi-hole/pi-hole/releases" rel="noopener" target="_blank">(Update available!)</a><?php } ?>
            <b>Web Interface Version </b><?php
            echo $web_current;
            if(isset($web_commit)) { echo " (".$web_branch.", ".$web_commit.")"; }
            if($web_update){ ?> <a class="alert-link animated infinite text-red lookatme" href="https://github.com/pi-hole/AdminLTE/releases" rel="noopener" target="_blank">(Update available!)</a><?php } ?>
            <b>FTL Version </b> <?php
            echo $FTL_current;
            if(isset($FTL_commit)) { echo " (".$FTL_branch.", ".$FTL_commit.")"; }
            if($FTL_update){ ?> <a class="alert-link animated infinite text-red lookatme" href="https://github.com/pi-hole/FTL/releases" rel="noopener" target="_blank">(Update available!)</a><?php } ?>
        </div>
